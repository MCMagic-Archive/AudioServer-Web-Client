<?php
	...
	$token = $_POST['token'];
	if($token!="Sfk0lzdSmywQmhSeXRS9uAYN"){
		die("");
	}
	$text = $_POST['text'];
	if(!isset($text)||$text==""||strpos($text, ' ') !== false){
		echo "Whoops, you made a typo! Just do /bans [Username/UUID]";
		die("");
	}
    ...
    $qry = "SELECT * FROM banned_players WHERE uuid=\"" . $uuid . "\" ORDER BY id DESC";
    $res = mysql_query($qry);
    if(!$res){
        echo "No Player Data has been found for that Player!";
        die("");
	...
	while ($row = mysql_fetch_array($res)) {
        array_push($attachments, array(
            "title" => ($row['permanent'] == 1 ? "Permanent" : "Temporary") . " Ban",
            "color" => ($row['permanent'] == 1 ? "danger" : "warning"),
            "fields" => array(
                array(
                    "title" => "Reason",
                    "value" => $row['reason'],
    ...
    if(empty($attachments)){
        echo "No bans found for this player!";
        die("");
    }
    $resp = array(
		"text" => "Ban History for " . $name,
