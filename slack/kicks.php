<?php
	...
	$token = $_POST['token'];
	if($token!="3NBuaK9LDCCm0R55woVD1EdX"){
		die("");
	}
	$text = $_POST['text'];
	if(!isset($text)||$text==""||strpos($text, ' ') !== false){
		echo "Whoops, you made a typo! Just do /kicks [Username/UUID]";
		die("");
	}
    ...
    $qry = "SELECT * FROM kicks WHERE uuid=\"" . $uuid . "\" ORDER BY id DESC";
    $res = mysql_query($qry);
    if(!$res){
        echo "No Player Data has been found for that Player!";
        die("");
	...
	while ($row = mysql_fetch_array($res)) {
        array_push($attachments, array(
            ...
            "fields" => array(
                ...
                array(
                    "title" => "Time",
                    "value" => date('n-j-Y g:i:s A', strtotime($row['time'])),
                    "short" => true
                )
            )
        ));
    ...
    if(empty($attachments)){
        echo "No kicks found for this player!";
        die("");
    }
    $resp = array(
		"text" => "Kick History for " . $name,
