...
function startConnecting() {
	console.log("Hey, you! We're always looking for some new developers, and you seem interested! Contact us at dev@mcmagic.us.");
    if (null == ws)
        if (ModalHelper.open("#connectingModal"), "WebSocket" in window) {
            var e = !1,
                t = "ws://198.24.160.154:" + port;
...
AudioManager._tabActive = !0, AudioManager._volume = .5, AudioManager._areas = [], AudioManager._fadeSteps = 10, AudioManager._extension = isIE || isSafari ? ".mp3" : ".ogg", AudioManager._folder_prefix = isIE || isSafari ? "mp3/" : "ogg/", AudioManager.locateFile = function (e) {
    ...
    var done = false;
    if(n.fadeAudio != 0){
        clearInterval(n.fadeAudio);
    }
    n.fadeAudio = setInterval(function () {
    ...
    if(i.fadeAudio != 0){
        clearInterval(i.fadeAudio);
    }
    i.fadeAudio = setInterval(function () {
    	if(sound.volume - step <= target){
    		sound.volume = target;
    	}else{
        	sound.volume -= step;
...
AudioManager.setMaximumVolume = function (e) {}, AudioManager.fadeTo = function (e, t, i) {}, AudioManager.playOnce = function (e, t, i) {
    ...
    var step = target * .01;
    if(o.fadeAudio != 0){
        clearInterval(o.fadeAudio);
    }
    o.fadeAudio = setInterval(function () {
    	if(sound.volume + step >= target){
    		sound.volume = target;
    	}else{
        	sound.volume += step;
